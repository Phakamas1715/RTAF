...
<body>
            ...
            <div class="rewards-info animate__animated animate__fadeInUp">
                ...
                <div class="reward-levels">
                    <div class="reward-item">
                        <div class="reward-points">30-50 คะแนน</div>
                        <div class="reward-desc">แลกรับรางวัล</div>
                    </div>
                    <div class="reward-item">
                        <div class="reward-points">100 คะแนน</div>
                        <div class="reward-desc">รางวัลพิเศษ </div>
                    </div>
                </div>
                <p class="reward-note">* คะแนนจะแสดงหลังจากสแกน QR Code ในแต่ละฐาน</p>
            </div>
    ...
    <script>
        ...
        function addChild() {
            ...
            childDiv.className = 'form-group animate__animated animate__fadeInUp';
            childDiv.id = `child-${childCount}`;
            childDiv.innerHTML = `
                <label>👶 ชื่อเด็กคนที่ ${childCount}</label>
                <input type="text" placeholder="กรุณากรอกชื่อเด็ก" required>
                <label>🎂 อายุ</label>
                <input type="number" min="1" max="15" placeholder="กรุณากรอกอายุ" required>
                <button type="button" onclick="removeChild(${childCount})" style="background: linear-gradient(45deg, #FF6B6B, #FF4949);">❌ ลบข้อมูล</button>
            `;
        ...
        function removeChild(id) {
            ...
            if (childDiv) {
                childDiv.classList.add('animate__fadeOutDown');
        ...
        document.getElementById('registrationForm').onsubmit = async function(e) {
            ...
            const children = Array.from(document.querySelectorAll('#childrenContainer > div')).map((child, index) => ({
                name: child.querySelector('input[type="text"]').value,
                age: child.querySelector('input[type="number"]').value,
            }));

            const uid = String(Math.floor(Math.random() * 9000) + 1000); // สร้างเลข 4 หลัก (1000-9999)

            // เก็บข้อมูลกิจกรรมที่เลือก
            const selectedActivities = Array.from(document.querySelectorAll('input[name="activities"]:checked')).map(checkbox => checkbox.value);
            ...
            try {
                const response = await fetch('https://script.google.com/macros/s/AKfycbyKpBYIXo1jp9wi5oDAv5WoskLCw4ykKIupP_57euLsN0FWV5JdIeHq3JowQnxTh6M/exec', {
                    method: 'POST',
                ...
                resultDiv.innerHTML = `
                    ...
                    <p>รหัสลงทะเบียนของคุณ:</p>
                    <p style="font-family: monospace; font-size: 32px; font-weight: bold; color: #4169E1; margin: 15px 0; letter-spacing: 3px;">${uid}</p>
                    <p>กรุณาแคปหน้าจอหรือจดจำรหัสนี้เพื่อใช้ยืนยันตัวตนสิทธิ์ในการรับรางวัล🎈✨</p>
                `;
